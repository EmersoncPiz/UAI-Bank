...
// SUGERENCIA DA IA: limpador de buffer
void limparBuffer() {
    int c;
    while ((c = getchar()) != '\n' && c != EOF);
}

//PARTE DE ALOCAÇĀO DE MEMÓRIA
// Função para aumentar a memória em 1 usuário
...
// Função para aumentar a memória para vários usuários
...
// Função para diminuir a memória em 1 usuário
...
// Função optimizadora para a opçāo 3 usando Busca Binária (inspiraçāo GEMA)
int buscaBinaria(Banco* banco, int id) { 
    ...
    while (inicio <= fim) {
        ...
        if (banco->lista[meio].id == id) {
            return meio; // usuario encontrado
        }
        if (banco->lista[meio].id < id) {
            inicio = meio + 1;
        } else {
    ...
    return -1; // se o usuario não foi encontrado
}
...
void opcao1(Banco* banco) {
    ...
    if (scanf(" %100[^,], %d, %lf", nome, &idade, &saldo) != 3) {
...
// Função para inserir vários usuários
void opcao2(Banco* banco) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (scanf(" %100[^,], %d, %lf", nome, &idade, &saldo) != 3) {
            printf("Erro: Entrada inválida. Formato esperado: (nome, idade, saldo)\n");
            limparBuffer();
            i--; // Decrementamos para repetir a leitura DESTE usuário específico
            continue;
        }
...
int main() {
    ...
    // liberação total da memória alocada durante o programa
    if (banco.lista != NULL) {
        free(banco.lista);
        banco.lista = NULL;
    }
